...
# ---- 3. 代码审查提醒（编辑计数器）----
EDIT_COUNT_FILE="$(pwd)/.claude/.edit-count-$(whoami)"
if [ "$TOOL_NAME" = "Write" ] || [ "$TOOL_NAME" = "Edit" ]; then
...
# Clean JSON output (use jq to safely escape WARNINGS)
if [ -n "$WARNINGS" ]; then
  jq -n --arg ctx "$WARNINGS" '{continue: true, hookSpecificOutput: {additionalContext: $ctx}}'
else
  echo '{"continue": true}'
fi
